...
def animation_frame(i):
    ...
    x_value = (i - base_time / 2.0) * 0.5
    y_value = math.asin(x_value)
...
fig_width = float(480) * px
fig_height = float(480) * px

# clear
fig, ax = plt.subplots(figsize=(fig_width, fig_height))
ax.xaxis.set_major_formatter(FuncFormatter(lambda val,pos: '{:.0g}'.format(val) if val !=0 else '0'))
...
# save to gif
anim.save("asin.gif", writer='pillow')
